...
const QuickPricing = () => {
  ...
  const toggleEditMode = (id, isEditing) => {
    setSalePrices((prev) => ({
      ...prev,
      [id]: isEditing
        ? products.find((p) => p._id === id)?.price.sale || ""
        : undefined,
    }));
  };
  ...
  const handleSubmit = async (e, id, dp) => {
    ...
    if (
      ...
      Number(salePrice) >= Number(dp)
    ) {
      return notify(
        "Sale price must be a number and less than the default price",
        "error"
      );
    }
    ...
    try {
      const response = await fetch(
        `https://backend.freshimeat.in/products/${id}`,
        {
          method: "PUT",
          headers: {
      ...
      if (response.ok) {
        setProducts((prevProducts) =>
          prevProducts.map((product) =>
            product._id === id
              ? { ...product, price: { ...product.price, sale: salePrice } }
              : product
          )
        );
  ...
  const makeFishOfTheDay = async (id) => {
    ...
    try {
      ...
      const url = `https://backend.freshimeat.in/ftds/${
        isBestseller ? ftds.find((ftd) => ftd?.product?._id === id)?._id : ""
      }`; // DELETE the ftd by its ID if exists
          ...
          {filteredProducts?.map((product) => (
            <tr key={product._id}>
              <td className="QP-product-info">
                <img
                  src={`https://backend.freshimeat.in/uploads/${product?.images[0]}`}
                  alt={product.name}
              ...
              <td className="QP-product-price">
                ...
                >
                  ...
                  <input
                    type="number"
                    value={
                      salePrices[product._id] !== undefined
                        ? salePrices[product._id]
                        : product.price.sale || ""
                    }
                    onChange={(e) =>
                  ...
                  <button
                    ...
                    className={`edit-btn ${
                      salePrices[product._id] !== undefined ? "apply-btn" : ""
                    }`}
                    onClick={(e) => {
              ...
              <td className="bestseller-checkbox">
                <form className="bestseller-checkbox">
                  ...
                  {product?.inStock && !product?.isArchived && (
                    <input
                    type="checkbox"
                    checked={ftds.some(
                      (ftd) => ftd?.product?._id === product?._id
                    )} // Check if product._id is in the ftds array
